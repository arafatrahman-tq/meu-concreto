...
describe("Nuvem Fiscal Provider", () => {
    ...
    describe("NuvemfiscalFiscalProvider", () => {
        it("should emit NF-e successfully", async () => {
            ...
            const coreSpy = spyOn(FiscalCore, "getContext").mockResolvedValue(mockContext);
        ...
        it("should handle errors during NF-e emission", async () => {
            const mockContext: any = {
                empresa: { id: 1, cnpj: "123", estado: "SP", empresa: "T", cidadeIbge: "1", crt: 1 },
            ...
            const coreSpy = spyOn(FiscalCore, "getContext").mockResolvedValue(mockContext);
            const apiSpy = spyOn(nuvemfiscalService, "emitNfe").mockRejectedValue(new Error("Erro de Conexão"));
        ...
        it("should emit NFS-e successfully", async () => {
            const mockContext: any = {
                empresa: { id: 1, cnpj: "12345678000199", estado: "SP", empresa: "Teste", cidadeIbge: "3550308", crt: 1 },
                cliente: { cpfCnpj: "12345678901", nome: "Teste", estado: "SP", cidadeIbge: "3550308", email: "a@a.com" },
                produto: { id: 1, produto: "Concreto", unidadeMedida: "m3" },
                orcamento: { id: 100, qtd: 1, total: 1000, valorUnit: 1000 },
                infoInsumos: "insumos",
                rules: { aliqISS: 0.03, aliqIBS: 0.001, aliqCBS: 0.009, aliqISS_retido: false }
            };
            ...
            expect(response.sucesso).toBe(true);
            expect(response.numero).toBe(500);
