...
describe('Utility Services (API Clients)', () => {
  ...
  describe('asaasService', () => {
    ...
    it('should handle creation errors in asaas', async () => {
      ...
      global.fetch = mock(async (url: string) => {
        ...
        return new Response(
          JSON.stringify({ errors: [{ description: 'Invalid CPF' }] }),
        )
      }) as any

      expect(
        asaasService.findOrCreateCustomer(idEmpresa, {
          name: 'Teste',
          cpfCnpj: '000',
        }),
  ...
  describe('sicoobService', () => {
    ...
    it('should handle auth failure in sicoob', async () => {
      ...
      expect(sicoobService.getToken(idEmpresa)).rejects.toThrow(
        'Erro de autenticação Sicoob',
      )
