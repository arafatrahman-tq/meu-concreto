    ...
    <div v-if="expenses && expenses.length > 0" class="space-y-10 flex-1">
      <div v-for="expense in expenses" :key="expense.name" class="space-y-3">
        ...
        <div class="h-3 w-full bg-primary/3 rounded-full overflow-hidden p-0.5">
          <div :style="{ width: expense.percentage + '%' }"
            class="h-full bg-brand rounded-full transition-all duration-1000 shadow-[0_0_10px_rgba(var(--brand-rgb),0.3)]">
          </div>
        </div>
      </div>
    </div>
    <div v-else class="flex-1 flex flex-col items-center justify-center opacity-20 py-10">
      <MoreHorizontal size="40" class="mb-2" />
      <p class="text-[10px] font-black uppercase tracking-widest">Sem dados de custos</p>
    </div>
