  ...
  <div
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12 relative z-10 shrink-0">
      <div>
        ...
        <p class="text-xl sm:text-3xl font-black tracking-tighter uppercase text-primary">Visão Geral de Ganhos</p>
      </div>
        ...
        <div v-for="(bar, index) in data" :key="index"
          ...
          <div
            ...
            <div :style="{ height: `${Math.max(bar.height, 8)}%` }"
              ...
              <!-- Shiny effect -->
              <div class="absolute top-0 inset-x-0 h-full bg-linear-to-b from-white/20 via-transparent to-black/10"></div>
            </div>
          </div>
