    ...
    <Teleport to="body">
        <Transition enter-active-class="transition-opacity duration-300 ease-out" enter-from-class="opacity-0"
            ...
            <div v-if="isOpen"
                ...
                <!-- Modal Content -->
                <div class="relative w-full transition-all duration-300 transform animate-in zoom-in-95 fade-in max-h-[95vh] flex flex-col" :class="[
                    size === 'sm' ? 'max-w-md' : size === 'lg' ? 'max-w-5xl' : size === 'xl' ? 'max-w-[95vw]' : 'max-w-2xl',
                    ...
                    <div class="flex justify-between items-start mb-8 flex-shrink-0">
                        <div>
                            <h3 class="text-xl md:text-2xl font-bold tracking-tight text-primary">{{ title }}</h3>
                            <p v-if="subtitle"
                    ...
                    <!-- Footer -->
                    <div v-if="$slots.footer" class="mt-8 pt-6 border-t border-border flex-shrink-0">
                        <slot name="footer" />
                    </div>
                </div>
            </div>
