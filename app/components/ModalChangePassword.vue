  ...
  <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <div
      ...
      <div class="mb-6 md:mb-8 text-center">
        <h3 class="text-xl md:text-2xl font-black uppercase tracking-tight text-primary">Segurança Exigida</h3>
        <p class="text-secondary mt-2 text-xs md:text-sm font-medium uppercase tracking-wider">Por
          favor, defina uma nova senha para continuar o acesso ao portal.</p>
      </div>

      <form @submit.prevent="handleSubmit" class="space-y-4 md:space-y-5">
        <div class="space-y-1">
          <label class="block text-[9px] md:text-[10px] font-black uppercase tracking-widest text-secondary ml-2">Senha
            Atual</label>
        ...
        <div class="space-y-1">
          <label class="block text-[9px] md:text-[10px] font-black uppercase tracking-widest text-secondary ml-2">Nova
            Senha</label>
        ...
        <div class="space-y-1">
          <label
            class="block text-[9px] md:text-[10px] font-black uppercase tracking-widest text-secondary ml-2">Confirmar
            Nova Senha</label>
        ...
        <button type="submit" :disabled="loading"
          ...
          <span v-if="loading"
            class="w-5 h-5 border-2 border-white/20 dark:border-black/20 border-t-white dark:border-t-black rounded-full animate-spin"></span>
...
const handleSubmit = async () => {
  ...
  try {
    ...
    logError('USUARIOS', `Erro ao atualizar senha do usuário #${props.userId}`, { error: err.message })
  } finally {
    loading.value = false
  }
}
