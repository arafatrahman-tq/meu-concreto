  ...
  <div class="w-full max-w-7xl mx-auto px-4 py-8">
    ...
    <div v-if="carregando" class="flex items-center justify-center py-20">
      <div class="text-center">
        <div class="w-12 h-12 border-4 border-brand/20 border-t-brand rounded-full animate-spin mx-auto mb-4"></div>
    ...
    <!-- Error State -->
    <div v-else-if="erro" class="bg-rose-50 dark:bg-rose-500/10 border border-rose-100 dark:border-rose-500/20 rounded-3xl p-8 text-center">
      ...
      <p class="text-sm font-medium text-secondary/60 mb-6">{{ erro }}</p>
      <button
        @click="carregarDados"
    ...
    <!-- Dashboard Content -->
    <div v-else-if="dadosCompletos" class="space-y-8">
      <!-- KPIs -->
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        >
          <div class="flex items-start justify-between mb-4">
            ...
            >
              <component :is="kpi.icon" class="w-6 h-6" :class="kpi.iconColorClass" />
            </div>
            <span 
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 bg-surface rounded-3xl border border-border shadow-[0_20px_50px_rgba(0,0,0,0.02)] p-6 transition-all duration-500 hover:shadow-[0_30px_70px_rgba(0,0,0,0.04)]">
          <div class="flex items-center gap-3 mb-6">
            ...
            <div>
              <h3 class="text-lg font-black text-primary tracking-tight">Vendas por Período</h3>
          ...
          <ClientOnly>
            ...
            <template #fallback>
              <div class="flex items-center justify-center h-75">
                <div class="w-8 h-8 border-4 border-brand/20 border-t-brand rounded-full animate-spin mr-3"></div>
        ...
        <div class="bg-surface rounded-3xl border border-border shadow-[0_20px_50px_rgba(0,0,0,0.02)] p-6 transition-all duration-500 hover:shadow-[0_30px_70px_rgba(0,0,0,0.04)]">
          <div class="flex items-center gap-3 mb-6">
            ...
            <div>
              <h3 class="text-lg font-black text-primary tracking-tight">Status</h3>
              <p class="text-[10px] font-black uppercase tracking-widest text-secondary/40">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div class="bg-surface rounded-3xl border border-border shadow-[0_20px_50px_rgba(0,0,0,0.02)] p-6 transition-all duration-500 hover:shadow-[0_30px_70px_rgba(0,0,0,0.04)]">
          <div class="flex items-center gap-3 mb-6">
            ...
            <div>
              <h3 class="text-lg font-black text-primary tracking-tight">Top Vendedores</h3>
              <p class="text-[10px] font-black uppercase tracking-widest text-secondary/40">
        ...
        <div class="bg-surface rounded-3xl border border-border shadow-[0_20px_50px_rgba(0,0,0,0.02)] p-6 transition-all duration-500 hover:shadow-[0_30px_70px_rgba(0,0,0,0.04)]">
          <div class="flex items-center gap-3 mb-6">
            ...
            <div>
              <h3 class="text-lg font-black text-primary tracking-tight">Top Clientes</h3>
              <p class="text-[10px] font-black uppercase tracking-widest text-secondary/40">
          ...
          <div class="space-y-3">
            ...
            >
              ...
              <div class="flex-1 min-w-0">
                <p class="font-bold text-primary truncate">{{ cliente.nomeCliente }}</p>
                <p class="text-[10px] font-black uppercase tracking-widest text-secondary/40">
                  {{ cliente.totalCompras }} compras
                </p>
              </div>
              <div class="text-right">
                <p class="font-black text-primary">{{ formatarMoeda(cliente.valorTotal) }}</p>
              </div>
            </div>
            <div v-if="!topClientes?.length" class="text-center py-8">
    ...
    <!-- Empty State -->
    <div v-else class="bg-surface border border-dashed border-border/60 rounded-3xl p-12 text-center">
      <BarChart3 class="w-16 h-16 text-secondary/20 mx-auto mb-4" />
      <h3 class="text-lg font-black text-primary mb-2">Nenhum dado disponível</h3>
      <p class="text-[10px] font-black uppercase tracking-widest text-secondary/40">
...
import type { KpisDashboard, VendaPorPeriodo, VendaPorVendedor, VendaPorCliente, StatusOrcamento } from '../../../shared/schemas/relatorios';
