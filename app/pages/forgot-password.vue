<template>
  <div class="relative min-h-screen flex items-center justify-center overflow-hidden bg-primary/2 font-sans">
    <!-- Background Decorativo -->
    <div class="absolute inset-0 z-0 opacity-40">
      <div class="absolute top-[-10%] left-[-10%] w-[40%] h-[40%] bg-brand/10 rounded-full blur-[120px]"></div>
    ...
    <div class="relative z-10 w-full max-w-lg px-6 flex flex-col items-center">
      ...
      <!-- Brand Header -->
      <div class="mb-10 text-center animate-in fade-in slide-in-from-top-10 duration-1000">
        <div
          ...
          >
            ...
            <path d="M30 38l25 6" stroke="white" stroke-width="2" stroke-linecap="round" opacity="0.5" />
            <!-- Wheels -->
        ...
        <h1 class="text-primary text-4xl font-bold tracking-tight mb-2">Meu Concreto</h1>
        <p class="text-secondary/60 text-xs font-semibold uppercase tracking-[0.2em]">Operational Intelligence</p>
      </div>
      ...
      <div
        ...
        <div v-if="!submitted">
          <div class="mb-10 text-center">
            <h2 class="text-2xl font-bold tracking-tight text-primary">Recuperar Acesso</h2>
            <p class="text-secondary text-sm font-medium mt-1 leading-relaxed">Informe seu e-mail ou usuário para
              receber as instruções de recuperação.</p>
          </div>

          <form @submit.prevent="handleSubmit" class="space-y-8">
            <div class="space-y-2">
              <label class="block text-xs font-bold text-secondary/80 ml-4">Identificação</label>
              <div class="relative group">
                <Mail
                  class="absolute left-5 top-1/2 -translate-y-1/2 text-secondary/40 group-focus-within:text-brand transition-colors"
                  size="18" />
                <input v-model="identity" type="text"
        ...
        <div v-else class="text-center py-4 animate-in fade-in zoom-in duration-500">
          ...
          <h3 class="text-2xl font-bold tracking-tight mb-4">Solicitação Enviada</h3>
          <p class="text-secondary text-base font-medium leading-relaxed mb-10">
            Se as informações coincidirem com nossa base, você receberá instruções de acesso em instantes.
          </p>
          <NuxtLink to="/login"
      ...
      <footer class="mt-12 text-center text-secondary/40 text-[10px] font-bold uppercase tracking-[0.2em]">
...
const handleSubmit = async () => {
  ...
  try {
    ...
    logError('AUTH', `Falha na solicitação de recuperação de senha: ${identity.value}`, { error: err.message })
    submitted.value = true
  } finally {
    loading.value = false
  }
