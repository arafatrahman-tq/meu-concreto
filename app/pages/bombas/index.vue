  ...
  <div class="flex flex-col gap-6 animate-enter">
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
      ...
      <div class="flex items-center gap-3 w-full md:w-auto">
        <div class="relative flex-1 md:flex-none">
          ...
          <input v-model="search" placeholder="BUSCAR BOMBA..."
    ...
    <div class="bg-surface rounded-3xl border border-border shadow-sm overflow-hidden">
      <BaseTable :headers="['Equipamento', 'Tipo', 'Placa', 'Status', '']">
        <tr v-for="bomba in filteredBombas" :key="bomba.id" class="group hover:bg-primary/2 transition-colors">
          <td class="px-8 py-5">
          ...
          <td class="px-8 py-5">
            <div class="flex items-center gap-2">
              <div :class="['w-2 h-2 rounded-full shadow-[0_0_10px]', bomba.ativo ? 'bg-emerald-500 shadow-emerald-500/50' : 'bg-red-500 shadow-red-500/50']"></div>
              <span class="text-[10px] font-black uppercase tracking-widest" :class="bomba.ativo ? 'text-emerald-500' : 'text-red-500'">
          ...
          <td class="px-8 py-5 text-right">
            <div class="flex items-center justify-end gap-2">
              <BaseTooltip text="Editar">
                <button @click="openModal(bomba)" class="p-2.5 rounded-xl bg-primary/3 text-secondary hover:text-brand hover:scale-110 active:scale-95 transition-all">
                  <Edit3 size="16" />
                </button>
              </BaseTooltip>
              <BaseTooltip text="Excluir">
                <button @click="deleteBomba(bomba.id)" class="p-2.5 rounded-xl bg-primary/3 text-secondary hover:text-red-500 hover:scale-110 active:scale-95 transition-all">
                  <Trash2 size="16" />
                </button>
              </BaseTooltip>
            </div>
      ...
      <div v-if="!filteredBombas.length && !pending" class="p-20 text-center">
        <Activity size="40" class="text-secondary opacity-10 mx-auto mb-4" />
        <p class="text-[10px] font-black uppercase tracking-[0.2em] text-secondary opacity-40">Nenhuma bomba cadastrada</p>
      </div>
    </div>

    <!-- Modal Form -->
    <BombaFormModal v-if="showModal" @close="showModal = false" @saved="refresh" :bomba="selectedBomba" />
  </div>
</template>
