<template>
  <div class="flex flex-col gap-6 animate-enter">
    ...
    >
      <BaseTable
        :headers="['Descrição', 'Prazo Médio', 'Integração Asaas', '']"
      >
        <tr
          v-for="forma in filteredFormas"
          :key="forma.id"
        ...
        >
          ...
          <td class="px-6 py-4">
            <span
              v-if="forma.tipoAsaas && forma.tipoAsaas !== 'UNDEFINED'"
            ...
            <span
              v-else
              class="text-[10px] font-black uppercase tracking-widest px-2.5 py-1 rounded-lg bg-gray-100 text-gray-400 border border-gray-200"
            >
              MANUAL
            </span>
          </td>
      ...
      <div v-if="!filteredFormas.length" class="p-20 text-center">
        ...
        <p class="text-secondary text-sm mt-1 font-medium">
          Experimente mudar o termo da busca ou adicionar uma nova condição.
        </p>
      </div>
    </div>
    ...
    <BaseModal
      ...
      :subtitle="
        isEditing
          ? 'Atualize as regras desta forma de pagamento'
          : 'Configure os prazos para novos orçamentos'
      "
      size="md"
    >
      <form @submit.prevent="saveForma" class="space-y-6">
        ...
        <div class="space-y-1.5">
          ...
          />
          <p
            class="text-[10px] text-secondary/40 ml-2 italic font-black uppercase"
          >
            Use vírgulas para múltiplas parcelas (ex: 30, 60, 90).
          </p>
        </div>
        ...
        <div class="space-y-1.5">
          ...
          <p
            class="text-[10px] text-secondary/40 ml-2 italic font-black uppercase"
          >
            Ao vincular, o sistema gerará a cobrança automaticamente no Asaas ao
            aprovar o orçamento.
          </p>
        </div>
        ...
        >
          <p
            class="text-rose-600 dark:text-rose-400 text-xs font-black text-center uppercase tracking-wider"
          >
            {{ error }}
          </p>
        </div>
        ...
        <div class="flex gap-4 pt-4">
          ...
          <button
            type="submit"
            :disabled="loading"
            class="flex-1 px-8 py-4 bg-brand text-white rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl shadow-brand/20 hover:scale-[1.02] active:scale-[0.98] transition-all disabled:opacity-50 outline-none"
          >
