<template>
    <div class="flex flex-col gap-6 animate-enter">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
                <h2 class="text-4xl font-black text-primary tracking-tighter uppercase">Produtos</h2>
        ...
        <div class="bg-surface rounded-3xl border border-border shadow-sm overflow-hidden">
            <BaseTable :headers="['Produto', 'Especificações', 'Preço Venda', 'Status', '']">
                <tr v-for="produto in displayedProdutos" :key="produto.id"
                    ...
                    <td class="px-8 py-5">
                        <div class="flex items-center gap-4">
                            ...
                            <div class="flex flex-col">
                                <span class="text-sm font-black uppercase tracking-tight text-primary">{{
                                    produto.produto }}</span>
                                <span class="text-[10px] font-black uppercase opacity-30 mt-0.5">{{
                    ...
                    <td class="px-8 py-5">
                        <div class="flex items-center gap-3">
                            <div v-if="produto.fck" class="flex flex-col">
                                <span
                                    class="text-[10px] font-black uppercase tracking-widest text-secondary opacity-40">FCK</span>
                                <span class="text-xs font-black uppercase text-primary tracking-tight">{{ produto.fck
                                }}</span>
                            </div>
                            <div v-if="produto.slump" class="flex flex-col">
                                <span
                                    class="text-[10px] font-black uppercase tracking-widest text-secondary opacity-40">Slump</span>
                                <span class="text-xs font-black uppercase text-primary tracking-tight">{{ produto.slump
                                }}</span>
                            </div>
                        </div>
                    </td>
                    <td class="px-8 py-5">
                        <div class="flex flex-col">
                            <span class="text-sm font-black uppercase tracking-tight text-brand">
                                {{ formatCurrency(produto.valorVenda) }}
                            </span>
                            <span v-if="produto.valorCusto"
                    ...
                    <td class="px-8 py-5">
                        <div class="flex items-center gap-2">
                            <div
                                :class="['w-2 h-2 rounded-full shadow-[0_0_10px]', produto.ativo ? 'bg-emerald-500 shadow-emerald-500/50' : 'bg-rose-500 shadow-rose-500/50']">
                            </div>
                            <span class="text-[10px] font-black uppercase tracking-widest"
                                :class="produto.ativo ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'">
                    ...
                    <td class="px-8 py-5">
                        <div class="flex items-center justify-end gap-2">
                            <BaseTooltip text="Mix Design (Traço)">
                                <button @click="openMixDesignModal(produto)"
                            ...
                            <BaseTooltip text="Editar">
                                <button @click="openEditModal(produto)"
            ...
            <div v-if="!filteredProdutos.length && !loading" class="p-20 text-center">
                ...
                <h3 class="text-lg font-black uppercase tracking-tight text-primary">Nenhum produto encontrado</h3>
        ...
        <div v-if="filteredProdutos.length > itemsToShow" class="flex justify-center py-8">
        ...
        <!-- Add/Edit Modal -->
        <BaseModal v-model="showModal" :title="isEditing ? 'Editar Produto' : 'Novo Produto'" size="lg">
            <form @submit.prevent="saveProduto" class="space-y-6 pt-4">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-2 col-span-2">
                        <label
                            class="text-[10px] font-black uppercase tracking-[0.2em] text-secondary ml-1 opacity-60">Nome
                            do Produto</label>
                        <BaseInput v-model="form.produto" placeholder="Ex: Concreto FCK 25 Mpa" :icon="Package"
                            required />
                    </div>

                    <div class="space-y-2">
                        <label
                            class="text-[10px] font-black uppercase tracking-[0.2em] text-secondary ml-1 opacity-60">Valor
                            Unitário Venda</label>
                        <BaseInput v-model.number="form.valorVenda" type="number" step="0.01" placeholder="0.00"
                            :icon="DollarSign" required />
                    </div>

                    <div class="space-y-2">
                        <label
                            class="text-[10px] font-black uppercase tracking-[0.2em] text-secondary ml-1 opacity-60">Valor
                            Unitário Custo</label>
                        <BaseInput v-model.number="form.valorCusto" type="number" step="0.01" placeholder="0.00"
                            :icon="TrendingDown" />
                    </div>

                    <div class="space-y-2">
                        <label
                            class="text-[10px] font-black uppercase tracking-[0.2em] text-secondary ml-1 opacity-60">Unidade
                            Medida</label>
                        <BaseSelect v-model="form.unidadeMedida" :options="unidadeOptions" :icon="Ruler" />
                    </div>

                    <div class="space-y-2">
                        <label
                            class="text-[10px] font-black uppercase tracking-[0.2em] text-secondary ml-1 opacity-60">FCK
                            (Mpa)</label>
                        <BaseInput v-model="form.fck" placeholder="Ex: 30" :icon="Activity" />
                    </div>

                    <div class="space-y-2">
                        <label
                            class="text-[10px] font-black uppercase tracking-[0.2em] text-secondary ml-1 opacity-60">Slump
                            (mm)</label>
                        <BaseInput v-model.number="form.slump" type="number" placeholder="Ex: 100" :icon="Layers" />
                    </div>

                    <div class="space-y-2">
                        <label
                            class="text-[10px] font-black uppercase tracking-[0.2em] text-secondary ml-1 opacity-60">Tipo
                            de Brita</label>
                        <BaseInput v-model="form.britaTipo" placeholder="Ex: Brita 1" :icon="Box" />
                    </div>

                    <div class="space-y-2">
                        <label
                            class="text-[10px] font-black uppercase tracking-[0.2em] text-secondary ml-1 opacity-60">Aditivo</label>
                        <BaseInput v-model="form.aditivo" placeholder="Ex: Plastificante" :icon="Activity" />
                    </div>

                    <div class="space-y-2 col-span-2">
                        <label
                            class="text-[10px] font-black uppercase tracking-[0.2em] text-secondary ml-1 opacity-60">Descrição
                            / Observações</label>
                        <BaseInput v-model="form.descricao" placeholder="Detalhes adicionais do produto..." />
                    </div>
                </div>

                <div class="p-6 bg-primary/2 rounded-3xl border border-border space-y-4">
                    <div class="flex items-center justify-between">
                        <div class="flex flex-col">
                            <span class="text-xs font-black uppercase tracking-tight text-primary">Produto Ativo</span>
                            <span class="text-[10px] font-black uppercase opacity-40">Disponível para novos
                                orçamentos</span>
                        </div>
                ...
                <div v-if="error" class="bg-rose-500/10 p-4 rounded-2xl border border-rose-500/20">
                    <p
                ...
                <div class="flex gap-4 pt-4">
                    ...
                    <button type="submit" :disabled="loading"
                        class="flex-1 py-4 bg-brand text-white rounded-2xl text-xs font-black uppercase tracking-widest hover:scale-[1.02] active:scale-[0.98] transition-all shadow-xl shadow-brand/20 disabled:opacity-50 flex items-center justify-center gap-2">
        ...
        <BaseDialog v-model="showDeleteDialog" title="Excluir Produto"
            :message="`Tem certeza que deseja remover ${produtoToDelete?.produto}? Esta ação não pode ser desfeita.`"
            variant="danger" @confirm="handleDelete" />

        <!-- Mix Design (Traço) Modal -->
        <BaseModal v-model="showMixModal" :title="`Mix Design: ${selectedProduto?.produto}`" size="xl">
            <div class="space-y-6 pt-4">
                <div class="p-6 bg-brand/5 border border-brand/10 rounded-3xl">
                    <div class="flex items-start gap-4">
                        ...
                        <div class="flex-1">
                            <h4 class="text-sm font-black uppercase tracking-tight text-primary">Composição do Produto
                            </h4>
                            <p class="text-[10px] font-black uppercase opacity-40 mt-1 leading-relaxed">
                                Defina os insumos e quantidades necessárias por unidade ({{ selectedProduto?.unidadeMedida
                                }}) de produto. Estas quantidades serão abatidas do estoque automaticamente.
                            </p>
                        </div>
                    </div>
                </div>

                <div v-if="loadingMix" class="py-12 flex flex-col items-center justify-center space-y-4">
                    <div class="w-8 h-8 border-4 border-brand/20 border-t-brand rounded-full animate-spin"></div>
                    <span class="text-[10px] font-black uppercase tracking-widest opacity-40">Carregando dados...</span>
                </div>
                ...
                <div v-else class="space-y-6">
                    ...
                    <div class="space-y-3">
                        <div class="flex items-center justify-between mb-2">
                            <span class="text-[10px] font-black uppercase tracking-[0.2em] text-secondary opacity-60">
                                Insumos Requeridos
                            </span>
                            <button @click="addInsumoAoTraco"
                        ...
                        <div v-if="tracoForm.itens.length === 0"
                            class="py-12 border-2 border-dashed border-border rounded-3xl flex flex-col items-center justify-center text-center px-6">
                            <div class="w-12 h-12 bg-primary/2 rounded-full flex items-center justify-center mb-4">
                                <Plus size="24" class="text-secondary/20" />
                            </div>
                            <span class="text-xs font-black uppercase text-primary tracking-tight">Nenhum insumo
                                adicionado</span>
                            <p class="text-[10px] font-black uppercase opacity-30 mt-1">Este produto ainda não possui um
                                traço definido.</p>
                        </div>

                        <div v-else class="space-y-3">
                            <div v-for="(item, index) in tracoForm.itens" :key="index"
                                class="flex items-center gap-4 bg-primary/2 p-4 rounded-2xl border border-border group">
                                <div class="flex-1">
                                    <BaseSelect v-model="item.idInsumo"
                                        :options="insumosDisponiveis.map(i => ({ label: `${i.nome} (${i.unidadeMedida})`, value: i.id }))"
                                        placeholder="Selecione o Insumo" />
                                </div>
                                <div class="w-40 relative group/input">
                                    <input v-model.number="item.quantidade" type="number" step="0.0001"
                                    ...
                                    <span
                                        ...
                                        {{ insumosDisponiveis.find(i => i.id === item.idInsumo)?.unidadeMedida || '' }}
                                    </span>
                                </div>
                                <button @click="removeInsumoDoTraco(index)"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="space-y-2">
                            <label
                                class="text-[10px] font-black uppercase tracking-[0.2em] text-secondary ml-1 opacity-60">Nome
                                do Traço</label>
                            <BaseInput v-model="tracoForm.nome" placeholder="Ex: Traço Padrão FCK 30" />
                        </div>
                        <div class="space-y-2">
                            <label
                                class="text-[10px] font-black uppercase tracking-[0.2em] text-secondary ml-1 opacity-60">Observação</label>
                            <BaseInput v-model="tracoForm.descricao" placeholder="Ex: Rendimento esperado..." />
                        </div>
                    </div>

                    <div class="flex gap-4 pt-4 border-t border-border">
                        <button type="button" @click="showMixModal = false"
                            class="flex-1 py-4 text-xs font-black uppercase tracking-widest text-secondary hover:bg-primary/3 rounded-2xl transition-all">
                            Fechar
                        </button>
                        <button @click="saveMixDesign" :disabled="loadingMix"
...
const { data: produtos, refresh, pending: loadingFetch } = await useFetch('/api/produtos')

const loading = ref(false)
watch(loadingFetch, (val) => { loading.value = val }, { immediate: true })
...
const openMixDesignModal = async (produto) => {
    ...
    try {
        ...
        addToast({ title: 'Erro', description: 'Não foi possível carregar os dados do traço.', type: 'error' })
    } finally {
...
const saveMixDesign = async () => {
    if (tracoForm.itens.length === 0) {
        addToast({ title: 'Atenção', description: 'Adicione pelo menos um insumo ao traço.', type: 'warn' })
        return
    }
    ...
    try {
        ...
        if (tracoForm.id) {
            await $fetch(`/api/tracos/${tracoForm.id}`, { method: 'PUT', body: payload })
        } else {
            await $fetch('/api/tracos', { method: 'POST', body: payload })
        }

        addToast({ title: 'Traço Salvo', description: 'A composição do produto foi atualizada.', type: 'success' })
        showMixModal.value = false
    } catch (err) {
        addToast({ title: 'Erro', description: 'Falha ao salvar traço.', type: 'error' })
    } finally {
        loadingMix.value = false
    }
}
...
const saveProduto = async () => {
    ...
    try {
        ...
        info('PRODUTOS', `${isEditing.value ? 'Edição' : 'Cadastro'} de produto: ${form.produto}`, { produto: payload })

        showModal.value = false
        refresh()
    } catch (err) {
        error.value = err.data?.statusMessage || 'Erro ao salvar produto'
        addToast({
            title: 'Erro',
            description: error.value,
            type: 'error'
        })
        logError('PRODUTOS', `Erro ao ${isEditing.value ? 'editar' : 'cadastrar'} produto`, { error: err.message, form })
    } finally {
        loading.value = false
    }
}
...
const handleDelete = async () => {
    try {
        await $fetch(`/api/produtos/${produtoToDelete.value.id}`, { method: 'DELETE' })
        addToast({
            title: 'Produto Removido',
            description: 'O registro foi excluído permanentemente.',
            type: 'success'
        })
        info('PRODUTOS', `Produto removido: ${produtoToDelete.value.produto}`, { id: produtoToDelete.value.id })
        refresh()
    } catch (err) {
        addToast({
            title: 'Erro',
        ...
        logError('PRODUTOS', `Erro ao remover produto: ${produtoToDelete.value?.produto}`, { error: err.message, id: produtoToDelete.value?.id })
    } finally {
