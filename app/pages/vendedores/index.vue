<template>
    <div class="flex flex-col gap-6 animate-enter">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
                <h2 class="text-4xl font-black text-primary tracking-tighter uppercase">Vendedores</h2>
        ...
        <div class="bg-surface rounded-3xl border border-border shadow-sm overflow-hidden">
            <BaseTable :headers="['Vendedor', 'Contato', 'Status', '']">
                <tr v-for="vendedor in displayedVendedores" :key="vendedor.id"
                    ...
                    <td class="px-8 py-5">
                        <div class="flex items-center gap-4">
                            ...
                            <span class="text-sm font-black uppercase tracking-tight text-primary">{{ vendedor.nome
                            }}</span>
                        </div>
                    </td>
                    <td class="px-8 py-5">
                        <div class="flex flex-col gap-0.5">
                            <span class="text-xs font-black uppercase tracking-widest text-primary">{{ vendedor.telefone
                                || '---' }}</span>
                    ...
                    <td class="px-8 py-5">
                        <div class="flex items-center gap-2">
                            <div
                                :class="['w-2 h-2 rounded-full shadow-[0_0_10px]', vendedor.ativo ? 'bg-emerald-500 shadow-emerald-500/50' : 'bg-rose-500 shadow-rose-500/50']">
                            </div>
                            <span class="text-[10px] font-black uppercase tracking-widest"
                                :class="vendedor.ativo ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'">
                    ...
                    <td class="px-6 py-4">
                        <div class="flex items-center justify-end gap-2">
                            <BaseTooltip text="Editar">
                                <button @click="openEditModal(vendedor)"
                            ...
                            <BaseTooltip text="Excluir">
                                <button @click="confirmDelete(vendedor)"
            ...
            <div v-if="!filteredVendedores.length" class="p-20 text-center">
                ...
                <h3 class="text-lg font-black uppercase tracking-tight text-primary">Nenhum vendedor encontrado</h3>
            ...
            <div v-else-if="itemsToShow < filteredVendedores.length" class="p-8 flex justify-center border-t border-border">
        ...
        <!-- Add/Edit Modal -->
        <BaseModal v-model="showModal" :title="isEditing ? 'Editar Vendedor' : 'Novo Vendedor'"
            :subtitle="isEditing ? 'Atualize os dados do vendedor' : 'Cadastre um novo vendedor para sua equipe'"
            size="md">
            <form @submit.prevent="saveVendedor" class="space-y-6">
                <div class="space-y-2">
                    <label class="text-xs font-bold text-secondary ml-1">Nome Completo</label>
                    <BaseInput v-model="form.nome" placeholder="Ex: Rogério Vendas" :icon="UserIcon" required />
                </div>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-2">
                        <label class="text-xs font-bold text-secondary ml-1">E-mail</label>
                        <BaseInput v-model="form.email" type="email" placeholder="vendedor@empresa.com" :icon="Mail" />
                    </div>
                    <div class="space-y-2">
                ...
                <div class="space-y-2">
                    <label class="text-xs font-bold text-secondary ml-1">PIN de Acesso Mobile (4 dígitos)</label>
                    <BaseInput v-model="form.pin" placeholder="Ex: 1234" :icon="Lock" maxlength="4" />
                    <p class="text-[10px] text-secondary/60 ml-1">Utilizado pelo vendedor para validar orçamentos no celular.</p>
                </div>

                <div class="p-6 bg-primary/2 rounded-3xl border border-border">
                    <BaseCheckbox v-model="form.ativo" label="Vendedor Ativo"
                        description="Vendedores inativos não aparecem na seleção de novos orçamentos." />
                </div>
        ...
        <BaseDialog v-model="showDeleteDialog" title="Excluir Vendedor"
...
const displayedVendedores = computed(() => filteredVendedores.value.slice(0, itemsToShow.value))

const openAddModal = () => {
    isEditing.value = false
    Object.assign(form, { id: null, nome: '', email: '', telefone: '', pin: '', ativo: true })
...
const saveVendedor = async () => {
    ...
    try {
        ...
        const url = isEditing.value ? `/api/vendedores/${form.id}` : '/api/vendedores'
        ...
        info('VENDEDORES', `${isEditing.value ? 'Edição' : 'Cadastro'} de vendedor: ${form.nome}`, { vendedor: payload })
        showModal.value = false
        refresh()
    } catch (err) {
        error.value = err.message
        logError('VENDEDORES', `Erro ao ${isEditing.value ? 'editar' : 'cadastrar'} vendedor`, { error: err.message, form })
    } finally {
        loading.value = false
    }
}
...
const handleDelete = async () => {
    try {
        ...
        info('VENDEDORES', `Vendedor removido: ${vendedorToDelete.value.nome}`, { id: vendedorToDelete.value.id })
        showDeleteDialog.value = false
        refresh()
    } catch (err) {
        addToast(err.message, 'error')
        logError('VENDEDORES', `Erro ao remover vendedor: ${vendedorToDelete.value?.nome}`, { error: err.message, id: vendedorToDelete.value?.id })
    }
}
</script>
