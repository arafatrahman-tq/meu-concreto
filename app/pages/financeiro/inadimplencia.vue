<template>
  <div class="flex flex-col gap-6 animate-enter">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
      <div>
        <h2 class="text-4xl font-black text-rose-500 tracking-tighter uppercase">Inadimplência</h2>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
      <div class="bg-surface p-6 rounded-3xl border border-border">
        <p class="text-[10px] font-black uppercase tracking-[0.2em] opacity-40 mb-1">Total em Atraso</p>
        <p class="text-3xl font-black text-rose-500 uppercase tracking-tighter">
          {{ formatCurrency(summary?.receber?.atrasado || 0) }}
        </p>
      </div>
      <div class="bg-surface p-6 rounded-3xl border border-border">
        <p class="text-[10px] font-black uppercase tracking-[0.2em] opacity-40 mb-1">Índice Geral</p>
        <p class="text-3xl font-black text-primary uppercase tracking-tighter">
          {{ summary?.indiceInadimplencia?.toFixed(1) || 0 }}%
        </p>
      </div>
      <div class="bg-surface p-6 rounded-3xl border border-border">
        <p class="text-[10px] font-black uppercase tracking-[0.2em] opacity-40 mb-1">Títulos Vencidos</p>
        <p class="text-3xl font-black text-primary uppercase tracking-tighter">
    ...
    <div class="bg-surface rounded-3xl border border-border shadow-sm overflow-hidden">
      <div class="px-8 py-6 border-b border-border bg-primary/2">
        <h3 class="text-xs font-black uppercase tracking-widest text-primary">Títulos em Atraso (Contas a Receber)</h3>
      </div>
      
      <BaseTable :headers="['Vencimento', 'Dias', 'Cliente', 'Venda', 'Valor', '']">
        <tr v-for="pgto in overduePayments" :key="pgto.id" class="group hover:bg-rose-500/5 transition-colors">
          <td class="px-8 py-5">
          ...
          <td class="px-8 py-5">
            <span class="px-2 py-1 rounded-lg bg-rose-500/10 text-rose-600 text-[10px] font-black uppercase">
              {{ getDaysOverdue(pgto.dataVencimento) }} dias
            </span>
          </td>
          <td class="px-8 py-5">
            <div class="flex flex-col">
              <span class="text-sm font-black uppercase text-primary">{{ pgto.venda?.orcamento?.nomeCliente || '---' }}</span>
              <span class="text-[10px] font-bold text-secondary opacity-40">{{ pgto.venda?.orcamento?.telefone || 'SEM TELEFONE' }}</span>
            </div>
          </td>
          <td class="px-8 py-5">
          ...
          <td class="px-8 py-5 text-right">
            <BaseTooltip content="Cobrar via WhatsApp">
              <button @click="openWhatsApp(pgto)" 
      ...
      <div v-if="overduePayments.length === 0" class="p-20 text-center">
        <CheckCircle2 size="48" class="text-green-500 mx-auto mb-4 opacity-20" />
        <p class="text-sm font-black uppercase tracking-widest text-secondary opacity-40">Nenhum título em atraso!</p>
      </div>
    </div>
  </div>
</template>
...
const overduePayments = computed(() => {
  ...
  return overdueData.value.filter(p => p.status === 'PENDENTE' && new Date(p.dataVencimento) < hoje)
})
