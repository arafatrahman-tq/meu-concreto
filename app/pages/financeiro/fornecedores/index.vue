  ...
  <div class="flex flex-col gap-6 animate-enter">
    ...
    >
      ...
      <div class="flex items-center gap-3 w-full md:w-auto">
        ...
        <button
          class="flex items-center gap-2 px-8 py-4 bg-brand text-white rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl shadow-brand/20 hover:scale-105 active:scale-95 transition-all outline-none"
          @click="openModal()"
        >
          <Plus size="18" />
    ...
    <!-- Table Container -->
    <div
      class="bg-surface rounded-3xl border border-border shadow-sm overflow-hidden min-h-100"
    >
      <div
        v-if="pending"
      ...
      <template v-else>
        ...
        >
          <tr
            v-for="item in displayedFornecedores"
            :key="item.id"
            class="group hover:bg-primary/1 transition-colors border-b border-border/50 last:border-0"
          >
            <td class="px-8 py-5">
              <span
                class="text-xs font-black text-secondary opacity-40 tabular-nums"
              >#{{ String(item.id).padStart(4, "0") }}</span>
            </td>
            <td class="px-8 py-5">
              <span
            ...
            <td
              class="px-8 py-5 text-xs font-black uppercase tracking-tight text-secondary opacity-60"
            >
              {{ item.contato || "---" }}
            </td>
            <td
        ...
        >
          ...
          <p
            class="text-sm font-medium text-secondary/60 max-w-xs mx-auto mt-2"
          >
            Não existem fornecedores que correspondam à sua busca ou não há
            registros.
          </p>
        </div>
        ...
        <div
          v-if="filteredFornecedores.length > itemsToShow"
      ...
      >
        <div class="grid grid-cols-1 gap-6">
          <div class="space-y-1.5">
            ...
            <BaseInput
              v-model="form.nome"
              placeholder="EX: CEMENTO EXPRESS LTDA"
              required
              :icon="Users"
            />
          </div>
        ...
        <div class="flex gap-3 pt-4">
          ...
          <button
            type="submit"
            :disabled="loading"
            class="flex-1 px-8 py-4 bg-brand text-white rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl shadow-brand/20 hover:scale-[1.02] active:scale-95 transition-all disabled:opacity-50 outline-none"
          >
...
const {
  ...
  pending,
  refresh,
} = await useFetch('/api/financeiro/fornecedores')
...
const saveFornecedor = async () => {
  try {
    ...
    info(
      'FINANCEIRO',
      `Fornecedor ${editingId.value ? 'atualizado' : 'cadastrado'}: ${form.value.nome}`,
      {
        fornecedor: form.value.nome,
    ...
    logError('FINANCEIRO', 'Erro ao salvar fornecedor', {
      error: error.message,
      form: form.value,
    })
    toast.error(
      error.data?.message
      || 'Não foi possível salvar os dados do fornecedor no momento.',
    )
  }
  finally {
    loading.value = false
...
const handleDelete = async () => {
  try {
    const nome = selectedFornecedor.value.nome
    await $fetch(
      `/api/financeiro/fornecedores/${selectedFornecedor.value.id}`,
      {
        method: 'DELETE',
      },
    )

    info('FINANCEIRO', `Fornecedor excluído: ${nome}`, {
      id: selectedFornecedor.value.id,
      nome,
    })
    ...
    logError('FINANCEIRO', 'Erro ao excluir fornecedor', {
      error: error.message,
      id: selectedFornecedor.value?.id,
    })
    toast.error(
      error.data?.message
      || 'Falha ao tentar remover o fornecedor selecionado.',
    )
  }
}
</script>
