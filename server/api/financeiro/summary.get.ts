...
export default defineEventHandler(async (event) => {
  ...
  try {
    const empresaIds =
      user.idEmpresasAcesso && user.idEmpresasAcesso.length > 0
        ? user.idEmpresasAcesso
        : [user.idEmpresa];
    ...
    const receivables = await db
      ...
      .where(
        and(
          inArray(pagamentos.idEmpresa, empresaIds),
    ...
    const payables = await db
      ...
      .where(
        and(
          inArray(contasPagar.idEmpresa, empresaIds),
