...
export default defineEventHandler(async (event) => {
  try {
    ...
    if (query.idEmpresa) {
      ...
      if (targetId !== user.idEmpresa && user.admin !== 1) {
        throw createError({
          statusCode: 403,
          message:
            'Apenas administradores podem visualizar dados de outras unidades',
        })
      }
    ...
    // Datas para comparação de tendências
    const now = new Date()
    ...
    if (totalAnterior > 0) {
      ...
      trendFaturamento =
        (diff >= 0 ? '+' : '') + ((diff / totalAnterior) * 100).toFixed(1) + '%'
    } else if (totalAtual > 0) {
      trendFaturamento = '+100%'
    }

    // Faturamento disponível (pagamentos já recebidos)
    ...
    const percentualFiscal =
      totalCount > 0 ? Math.round((comNotaCount / totalCount) * 100) : 0

    // Cálculo de custos (valorCusto * qtd)
    ...
    // Dados para o gráfico baseados no período
    let chartSelect
    let chartWhere = and(
      eq(vendas.idEmpresa, idEmpresa),
      isNull(vendas.deletedAt)
    ...
    // Agenda de entregas (próximas 5 entregas/pedidos com data de entrega)
    ...
    if (volAnterior > 0) {
      ...
      trendVolume =
        (diff >= 0 ? '+' : '') + ((diff / volAnterior) * 100).toFixed(1) + '%'
    }
    ...
    const ticketMedio =
      countVendas > 0 ? Math.round(totalVendas / countVendas) : 0
    ...
    const conversao =
      totalOrcamentos > 0
        ? Math.round((countVendas / totalOrcamentos) * 100)
        : 0
    ...
    return {
      ...
      recentDeliveries: recentDeliveries.map((d: any) => ({
        ...
        time: d.dataEntrega
          ...
          : '00:00',
        date: d.dataEntrega
          ? new Date(d.dataEntrega).toISOString().split('T')[0]
          : '',
